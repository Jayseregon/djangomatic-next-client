...
import DocsAdminPage from "@/app/docs/admin/[slug]/page";
...
describe("DocsAdminPage", () => {
  const mockProps = {
    params: Promise.resolve({ slug: "test-doc" }),
  };

  const renderDocsAdminPage = async (session: any = null) => {
    const { auth } = require("../../../../../auth");

    (auth as jest.Mock).mockImplementation(() => Promise.resolve(session));

    return render(await DocsAdminPage(mockProps));
  };
  ...
  it("shows UnAuthenticated component when no session exists", async () => {
    await renderDocsAdminPage(null);
  ...
  it("renders DynamicDocTemplate when authenticated", async () => {
    ...
    await renderDocsAdminPage(mockSession);

    const template = screen.getByTestId("dynamic-doc-template");

    expect(template).toBeInTheDocument();
    expect(template).toHaveAttribute("data-doc-type", "admin");
    expect(template).toHaveAttribute("data-permission", "canAccessDocsAdmin");
    expect(template).toHaveAttribute("data-email", "admin@example.com");
    expect(template).toHaveAttribute("data-slug", "test-doc");
  });

  describe("Metadata Generation", () => {
    it("generates metadata correctly", async () => {
      const { generateMetadata } = require("@/app/docs/admin/[slug]/page");
  ...
  describe("Static Params Generation", () => {
    it("generates static params correctly", async () => {
      const { generateStaticParams } = require("@/app/docs/admin/[slug]/page");
