...
function SaasPageContent({ session }: { session: any }): JSX.Element {
  ...
  const endpoint = "/saas/tds/ajax/query-node-dfn-pattern-list/";

  return (
    <WithPermissionOverlay
      email={session.user.email}
      permission="canAccessAppsTdsSuper"
    >
      <div className="space-y-5 mb-5">
        <InputDataProvider>
          ...
          <DatabaseSchemaTable3Selector
            ...
            endpoint={endpoint}
            pattern="*"
            tableDescription="version_pattern"
          />
          <SuperVersionControl btnHelper="version_uniq" />
