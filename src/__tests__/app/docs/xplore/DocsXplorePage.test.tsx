...
import DocsXplorePage from "@/app/docs/xplore/[slug]/page";
...
describe("DocsXplorePage", () => {
  const mockProps = {
    params: Promise.resolve({ slug: "test-doc" }),
  };

  const renderDocsXplorePage = async (session: any = null) => {
    const { auth } = require("../../../../../auth");

    (auth as jest.Mock).mockImplementation(() => Promise.resolve(session));

    return render(await DocsXplorePage(mockProps));
  };
  ...
  it("shows UnAuthenticated component when no session exists", async () => {
    await renderDocsXplorePage(null);
  ...
  it("renders DynamicDocTemplate when authenticated", async () => {
    ...
    await renderDocsXplorePage(mockSession);

    const template = screen.getByTestId("dynamic-doc-template");

    expect(template).toBeInTheDocument();
    expect(template).toHaveAttribute("data-doc-type", "xplore");
    expect(template).toHaveAttribute("data-permission", "canAccessDocsXplore");
  ...
  describe("Metadata Generation", () => {
    it("generates metadata correctly", async () => {
      const { generateMetadata } = require("@/app/docs/xplore/[slug]/page");
  ...
  describe("Static Params Generation", () => {
    it("generates static params correctly", async () => {
      const { generateStaticParams } = require("@/app/docs/xplore/[slug]/page");
