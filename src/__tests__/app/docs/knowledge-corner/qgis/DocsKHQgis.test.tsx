...
import DocsKHQgisPage from "@/app/docs/knowledge-corner/qgis/[slug]/page";
...
describe("DocsKHQgisPage", () => {
  const mockProps = {
    params: Promise.resolve({ slug: "test-doc" }),
  };

  const renderDocsKHQgisPage = async (session: any = null) => {
    const { auth } = require("../../../../../../auth");

    (auth as jest.Mock).mockImplementation(() => Promise.resolve(session));

    return render(await DocsKHQgisPage(mockProps));
  };
  ...
  it("shows UnAuthenticated component when no session exists", async () => {
    await renderDocsKHQgisPage(null);
  ...
  it("renders DynamicDocTemplate when authenticated", async () => {
    ...
    await renderDocsKHQgisPage(mockSession);
  ...
  describe("Metadata Generation", () => {
    it("generates metadata correctly", async () => {
      const {
        generateMetadata,
      } = require("@/app/docs/knowledge-corner/qgis/[slug]/page");
