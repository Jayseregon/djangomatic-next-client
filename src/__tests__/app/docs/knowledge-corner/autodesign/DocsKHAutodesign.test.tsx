...
import DocsKHAutodesignPage from "@/app/docs/knowledge-corner/autodesign/[slug]/page";
...
describe("DocsKHAutodesignPage", () => {
  const mockProps = {
    params: Promise.resolve({ slug: "test-doc" }),
  };

  const renderDocsKHAutodesignPage = async (session: any = null) => {
    const { auth } = require("../../../../../../auth");

    (auth as jest.Mock).mockImplementation(() => Promise.resolve(session));

    return render(await DocsKHAutodesignPage(mockProps));
  };
  ...
  it("shows UnAuthenticated component when no session exists", async () => {
    await renderDocsKHAutodesignPage(null);
  ...
  it("renders DynamicDocTemplate when authenticated", async () => {
    ...
    await renderDocsKHAutodesignPage(mockSession);
  ...
  describe("Metadata Generation", () => {
    it("generates metadata correctly", async () => {
      const {
        generateMetadata,
      } = require("@/app/docs/knowledge-corner/autodesign/[slug]/page");
