...
import SaasPage from "@/app/saas/xplore/admin/extract_bom_summary/page";
...
describe("SaasXploreAdminExtractBomSummary Page", () => {
  const mockMessages = {
    ServerDropdowns: {
  ...
  describe("Page Components", () => {
    ...
    it("passes correct props to components", async () => {
      ...
      // Check AppPageTitle props
      const title = screen.getByTestId("app-page-title");

      expect(title).toHaveAttribute("data-client", "xplore_saas");

      // Check AppPageDescription props
      const description = screen.getByTestId("app-page-description");

      expect(description).toHaveAttribute("data-client", "xplore_saas");
      expect(description).toHaveAttribute("data-translation", "xploreApps");
    });
