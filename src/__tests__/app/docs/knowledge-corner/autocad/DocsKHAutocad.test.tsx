...
import DocsKHAutocadPage from "@/app/docs/knowledge-corner/autocad/[slug]/page";
...
describe("DocsKHAutocadPage", () => {
  const mockProps = {
    params: Promise.resolve({ slug: "test-doc" }),
  };

  const renderDocsKHAutocadPage = async (session: any = null) => {
    const { auth } = require("../../../../../../auth");

    (auth as jest.Mock).mockImplementation(() => Promise.resolve(session));

    return render(await DocsKHAutocadPage(mockProps));
  };
  ...
  it("shows UnAuthenticated component when no session exists", async () => {
    await renderDocsKHAutocadPage(null);
  ...
  it("renders DynamicDocTemplate when authenticated", async () => {
    ...
    await renderDocsKHAutocadPage(mockSession);

    const template = screen.getByTestId("dynamic-doc-template");

    expect(template).toBeInTheDocument();
    expect(template).toHaveAttribute("data-doc-type", "knowledge-corner");
    expect(template).toHaveAttribute("data-permission", "canAccessDocsKC");
  ...
  describe("Metadata Generation", () => {
    it("generates metadata correctly", async () => {
      const {
        generateMetadata,
      } = require("@/app/docs/knowledge-corner/autocad/[slug]/page");
